const profilesData = {
    ...
    'fallou_wade': {
        ...
        email: 'fallou.wade@email.com',
        image: 'mo.png',
        experiences: [
            'Développeur Full-Stack (2022-2023) : Conception et développement d\'applications web et mobiles.'
        ],
        projects: [
            'Projet de fin d\'études : Plateforme de gestion des événements universitaires.'
        ],
        competences: [
            'Programmation (Python, Java, C++)',
            'Développement Web (HTML, CSS, JavaScript, Node.js)',
            'Bases de données (SQL, MongoDB)'
        ],
        loisirs: [
            'Lecture, football, jeux vidéo.'
        ],
        contacts: [
            'LinkedIn : linkedin.com/in/fallou-wade',
            'GitHub : github.com/fallou-dev'
        ]
    },
    'serge_senghor': {
        name: 'Serge Senghor',
...
function injectData(profileId) {
    ...
    if (profileId === 'fallou_wade') {
        const addressElement = document.getElementById('address-contact-fallou');
        if (addressElement) addressElement.textContent = profile.address;
        
        const footerEmailElement = document.getElementById('footer-email-fallou');
        if (footerEmailElement) footerEmailElement.textContent = profile.email;

        const profileImage = document.getElementById('profile-photo-fallou_wade');
        if (profileImage) profileImage.src = `Image/${profile.image}`;

        const nameContact = document.getElementById('name-fallou_wade');
        if (nameContact) nameContact.textContent = `Nom: ${profile.name.split(' ')[1]} Prénom: ${profile.name.split(' ')[0]}`;

        const phoneContact = document.getElementById('phone-contact-fallou');
        if (phoneContact) phoneContact.textContent = profile.phone;

        const emailContact = document.getElementById('email-contact-fallou');
        if (emailContact) emailContact.textContent = profile.email;

        const experienceList = document.getElementById('experience-list-fallou');
        if (experienceList) {
        ...
        const projectsList = document.getElementById('projects-list-fallou');
        if (projectsList) {
        ...
        const competencesList = document.getElementById('competences-list-fallou');
        if (competencesList) {
        ...
        const loisirsList = document.getElementById('loisirs-list-fallou');
        if (loisirsList) {
        ...
        const contactsList = document.getElementById('contacts-list-fallou');
        if (contactsList) {
